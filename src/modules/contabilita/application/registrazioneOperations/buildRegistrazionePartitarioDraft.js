...
export function buildRegistrazionePartitarioDraft(input = {}, options = {}) {
  ...
  const importoAperto = isApertura
    ? (manualOpenOverride
        ...
        : documentTotal)
    : 0
  const residuo = isApertura ? importoAperto : selection.saldoResiduo
  const importoChiusura = isChiusura
    ? (manualCloseOverride
        ? toAmount(currentPartitarioDraft.importoChiusura)
        : selection.importoChiusura)
    : 0
  ...
  const draft = {
    ...
    rows: isApertura
      ...
      : openItems.map((item, index) => {
          const rowResiduo = toAmount(item?.saldoResiduo ?? item?.saldo_residuo ?? item?.importo_residuo ?? item?.residuo ?? item?.saldo ?? 0)
          return {
            ...
            importoOriginario: toAmount(item?.importoOrigine ?? item?.importo_origine ?? item?.importo_originale ?? item?.totale ?? 0),
            importoAperto: 0,
            importoChiusura: item?.importoChiusura ?? 0,
            residuo: rowResiduo,
            segno: item?.segno || inferSign('chiusura', rowResiduo),
            stato: normalizeText(item?.stato || 'aperta'),
            selected: String(item?.id || '').trim() === normalizeText(currentPartitarioDraft.selectedPartitaId || selectedPartita?.id),
          }
        }),
  ...
  return {
    ...
    totals: {
      ...
      residuo: toAmount(residuo),
      saldoResiduo: isChiusura ? selection.saldoResiduo : toAmount(residuo),
    },
  }
}
