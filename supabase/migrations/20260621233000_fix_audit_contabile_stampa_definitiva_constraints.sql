-- Migration patch to fix check constraints on audit_contabile for Stampa Definitiva
-- Timestamp: 20260621233000

-- 1. Aggiornamento check constraint per operation_type
alter table public.audit_contabile
  drop constraint if exists audit_contabile_operation_type_ck;

alter table public.audit_contabile
  add constraint audit_contabile_operation_type_ck
  check (operation_type in ('INSERT', 'UPDATE', 'ANNULLA', 'STORNO', 'RETTIFICA', 'STAMPA_DEFINITIVA'));

-- 2. Aggiornamento check constraint per source_module
alter table public.audit_contabile
  drop constraint if exists audit_contabile_source_module_ck;

alter table public.audit_contabile
  add constraint audit_contabile_source_module_ck
  check (source_module in ('registrazione_manual', 'import_contabilita', 'riconciliazione_bancaria', 'stampe_definitive'));

-- 3. Stored Procedure Aggiornata
...
begin
  ...
  insert into public.audit_contabile (
    ...
    p_societa_id, 'stampe_definitive', v_stampa_id, 'STAMPA_DEFINITIVA',
    ...
    jsonb_build_object(
      ...
      'rows_count', v_riga_count,
      'totale_complessivo', v_totale_complessivo
    ),
    p_creato_by, now(), 'stampe_definitive'
  );
