// Generazione stampe HTML/PDF contabili.
...
export async function generateStampeHandler({ body }) {
  const { tipo, societa, dati, periodo } = body || {}
  ...
  try {
    ...
    if (tipo === 'registro_iva') {
      const { registroTipo, movimenti } = dati
      ...
      const rows = conti.filter((c) => c.saldo_dare > 0 || c.saldo_avere > 0).map((c) => {
      ...
      const { conti, tipoSituazione } = dati
      ...
      const { partite, tipoPartitario } = dati
      ...
      return { status: 400, json: { error: 'Tipo stampa non valido' } }
    }
    ...
    console.error('[generateStampeHandler]', error)
    return {
      status: 500,
      json: {
        error: 'Errore generazione stampa',
